...
function renderMarketBots() {
    db.ref('bots').once('value', snapshot => {
        ...
        if(!snapshot.exists()) { 
            market.innerHTML = '<p style="color:#64748b; text-align:center; font-size:13px;">মার্কেটে কোনো বট উপলব্ধ নেই।</p>'; 
            return; 
        }

        snapshot.forEach(child => {
            let bot = child.val();
            // এখানে বটের মেয়াদ (days) ডাইনামিকালি যুক্ত করা হয়েছে
            market.innerHTML += `
                <div class="bot-card" style="background:#1e293b; padding:15px; border-radius:8px; margin-bottom:12px; border:1px solid #334155;">
