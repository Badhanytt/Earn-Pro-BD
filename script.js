...
// ৩, ৪. সিকিউর ডিভাইস রেজিস্ট্রি এবং কোড ভিত্তিক রেজিষ্ট্রেশন
function handleAuth() {
    ...
    db.ref('users/' + currentUser.username).on('value', snapshot => {
        ...
        if(currentUser) {
            ...
            if(document.getElementById('user-ref-pending')) {
                ...
                // প্রোফাইলে ব্যবহারকারীর স্থায়ী পার্মানেন্ট কোডটি বসানো
                document.getElementById('permanent-ref-code').value = currentUser.myOwnRefCode || "N/A";
            }
...
// পার্মানেন্ট কোড কপি করার ফাংশন
function copyRefCode() {
    let copyText = document.getElementById("permanent-ref-code");
    if(copyText.value === "N/A" || copyText.value === "") return alert("Code not loaded yet!");
    copyText.select();
    copyText.setSelectionRange(0, 99999);
    navigator.clipboard.writeText(copyText.value);
    alert("Your permanent refer code copied: " + copyText.value);
}
...
function logout() {
    ...
    clearInterval(leaderboardInterval); // <-- এই লাইনটি নতুন যুক্ত করা হয়েছে
    currentUser = null;
...
// লিডারবোর্ডের গ্লোবাল ইন্টারভাল ট্র্যাকার ভ্যারিয়েবল (ফাইলের একদম উপরে অন্যান্য ভ্যারিয়েবলের সাথে রাখতে পারেন)
...
function loadLeaderboard() {
    ...
    // ১. ডাটাবেজ পুলে থাকা সব মেম্বারদের ফেইক ডাটা বেস (এখান থেকে প্রতিবার র্যান্ডমাইজ হবে)
    const allUsersPool = [
        { name: "Md. Rafiqul Islam", phone: "01734******", baseEarn: 5420, baseRef: 48, tag: 'কিং মেম্বার' },
    ...
    // র্যান্ডম ১০ জন সিলেক্ট ও রেন্ডার করার কোর ইঞ্জিন
    function renderRandomTopTen() {
        // অ্যারে স্ক্র্যাম্বলার লজিক (Array Shuffler)
        let shuffled = allUsersPool.sort(() => 0.5 - Math.random());
        
        // প্রথম ১০ জন ইউজারকে কেটে নেওয়া হলো
        let selectedTen = shuffled.slice(0, 10);

        // বেশি আয়ের ওপর ভিত্তি করে বড় থেকে ছোট ক্রমানুসারে সাজানো (Sort descending by earnings)
        selectedTen.sort((a, b) => b.baseEarn - a.baseEarn);

        // কন্টেইনার খালি করে অ্যানিমেশন সহ রেন্ডার করা
        ...
        selectedTen.forEach((u, index) => {
            ...
            // র‍্যাঙ্কিং ডিজাইন কাস্টমাইজেশন
            if (rank === 1) {
                rankClass = 'rank-1'; 
    ...
    // প্রথমবার পেজ লোড হওয়ার সাথে সাথে ১ বার রান হবে
    renderRandomTopTen();

    // পূর্বের কোনো ইন্টারভাল চলতে থাকলে তা ক্লিয়ার করা (যাতে মেমোরি লিক বা ডাবল লুপ না হয়)
    if(leaderboardInterval) clearInterval(leaderboardInterval);

    // ঠিক প্রতি ১০ সেকেন্ড (১০০০০ মিলি-সেকেন্ড) পর পর স্বয়ংক্রিয়ভাবে ডাটা চেঞ্জ হবে
...
function calculateAdminNetCash() {
    setTimeout(() => {
        ...
        let wit = parseFloat(document.getElementById('stat-total-withdrawn').innerText) || 0;
        let container = document.getElementById('stat-admin-cash');
        if(container) container.innerText = (dep - wit).toFixed(2);
    }, 300);
}
